...
"""Task artifact schemas for Phase 1.

The artifact plane is the source of truth for the active workflow.
These models are deliberately explicit so stage gates can be enforced mechanically.
"""
...
class TaskStateRecord(Timestamped):
    task_id: str
    state: TaskState
...
class EvidencePack(Timestamped):
    task_id: str
    summary: str
...
class DesignReview(Timestamped):
    task_id: str
...
class SelfTestReport(Timestamped):
    task_id: str
...
class ValidationReport(Timestamped):
    task_id: str
    passed: bool
    summary: str
...
class ExperiencePacket(Timestamped):
    task_id: str
    project_id: str
    summary: str
    env: EnvTuple = Field(default_factory=EnvTuple)
