...
class Phase2Store:
    ...
    def refine_writeback(self, payload: dict[str, Any]) -> RefinedWriteback:
        ...
        if persist:
            for item in objects:
                persisted = self.canonical_store.create_candidate(item)
                object_ids.append(persisted["id"])
        else:
            object_ids = [item["id"] for item in objects]

        now = utc_now()
        summary = f"Refined {len(objects)} candidate object(s) from task {packet.task_id}"
        return RefinedWriteback(
            task_id=packet.task_id,
            persisted=persist,
            persist_target="candidate" if persist else None,
            object_status="candidate" if objects else None,
            summary=summary,
    ...
    def _load_experience_packet(self, payload: dict[str, Any]) -> ExperiencePacket:
        ...
        active_path = self.tasks_root / "active" / task_id / "80_writeback" / "experience-packet.json"
        legacy_path = self.tasks_root / task_id / "80_writeback" / "experience-packet.json"
        path = active_path if active_path.exists() or (self.tasks_root / "active").exists() else legacy_path
        if not path.exists():
