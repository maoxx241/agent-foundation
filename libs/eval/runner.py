...
class EvaluationRunner:
    ...
    def run_gold_case(self, case: GoldQueryCase, *, run_id: str) -> RetrievalCaseResult:
        workspace_root = self.repo_root / "shadow_runs" / run_id / "gold" / case.case_id
    ...
    def run_replay_case(self, case: ReplayCase, *, run_id: str) -> ReplayCaseResult:
        workspace_root = self.repo_root / "shadow_runs" / run_id / "replay" / case.case_id
        env = self._make_environment(workspace_root)
        try:
    ...
    def _make_environment(self, workspace_root: Path) -> dict[str, Any]:
        ...
        artifact_client = TestClient(create_artifact_app(artifact_store, Observability(observability_root / "artifact_api")))
        kb_client = TestClient(create_kb_app(kb_store, phase2_store, Observability(observability_root / "thin_kb_api")))
        return {
